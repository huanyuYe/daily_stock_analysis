...
- [修复] A 股主动推送与被动报告脚本从任意工作目录启动时会显式引导项目根路径，分析显式失败经共享锁传播为非零退出，避免 systemd 在导入失败或个股流水线失败后误报成功
- [修复] 港股/美股阶段报告推送前校验预期标的与摘要/详情覆盖，保存同名 `.run.json` 清单并记录投递成功/失败，在缺失、重复、意外标的、解析失败或 QQ 未确认成功时拒绝把任务标为完成
- [修复] `codex_cli` 普通生成忽略宿主用户工具配置并关闭 shell/web/image 工具，保留既有登录态、只读沙箱和脱敏诊断，避免无人值守报告因工具批准请求中断
- [改进] AnalysisContextPack 的核心行情、日线或技术块降级时对整体质量等级应用 `limited/poor` 上限，保留原始加权分与 limitations，并把买入/加仓报告及低质量 DecisionSignal 确定性降级为观望，避免辅助数据高分掩盖关键输入不足
- [改进] DecisionSignal 后验升级为并存的 v2 口径，支持明确趋势的 watch/alert 和收盘后 intraday 验证，并在启用自动回测时随成功分析批量推进缺失或可重试 outcome
- [修复] 跨市场主线必需上游报告/快照缺失时返回失败退出码而非绿色跳过，并区分 adapter 通用 stale 元数据与本报告交易时段窗口的新鲜度判断
- [新功能] 新增每日 09:25 美股收盘催化映射与 16:50 A/HK 收盘验证两轮跨市场主线报告，复用精选资讯、SEC/HKEX 官方披露、行业/概念榜单和现有逐股报告，保存可审计 JSON 快照并在关键输入陈旧或缺失时拒绝推送
- [修复] 港股和美股大盘复盘不再将腾讯指数行情中的重复成交量或“指数点位乘成交量”误当货币成交额；缺失时明确显示 N/A，并按市场动态标注成交额单位
- [修复] 行情快照将现价、昨收、涨跌额和涨跌幅按同一实时报价原子化归一；实时报价缺少昨收时优先由同源涨跌额或涨跌幅反推，避免与不同交易日历史行拼接后出现涨跌方向相反
- [修复] A 股盘前、午间和盘后任务在保留根目录兼容报告的同时归档到独立阶段目录，并记录 20 分钟交付 SLO，避免同日三轮报告互相覆盖且不通过截断数据换取耗时
- [改进] 公共 SearXNG 对 429、访问验证、5xx 和超时实例增加进程内分级冷却，继续轮询健康候选和其他搜索源，避免同一轮报告为已知失败端点重复消耗等待时间
- [改进] SEC ticker 映射遇到 403 后在当前进程内进入 30 分钟失败冷却并增加联系邮箱配置告警，避免每个美股重复访问同一被拒绝端点，同时继续显式标记官方披露不可用
- [修复] 港股和美股定时报告目标改为 Futu 正股持仓与 `STOCK_LIST` 关注标的的去重并集，避免已配置的腾讯 `HK00700` 因未持仓而从例行报告中缺失
- [改进] Tushare 行业排行的明确接口权限拒绝按具体 endpoint 在当前进程内记忆，后续标的直接进入原有 fallback，避免重复请求静态不可用套餐接口
- [改进] 离岸市场基本面估值复用主流程实时行情，避免港股与美股每只标的重复执行行情 fallback
- [修复] 最终动作与作战计划新增一致性收敛：持仓未知或空仓时不再输出“持有”，中性动作的买入点改为条件参考且不再保留直接建仓/加仓建议，减仓/卖出移除入场计划；QQ 精简报告按统一八态动作归类，避免把“持有”错误计入卖出并显示红色图标
- [改进] SEC/HKEXnews 与美股财报期权新增同主机请求间隔、429 冷却及跨进程 last-good 缓存；美股期权可显式启用现有 Futu OpenD 只读期权链/快照作为 Yahoo 失败备用源；新鲜请求失败时保留完整数据字段并显式标注陈旧时间和失败原因，不以压缩高权重数据丰富度换取报告耗时
...
- [新功能] 美股关注标的新增财报日与邻近到期期权分析，盘前、盘中、盘后分别标注财报事件窗口、正股变化、期权量/OI、成本、盈亏平衡和模型盈利概率。
...
- [新功能] A 股个股分析接入巨潮资讯公告与东方财富个股新闻的结构化事件上下文，并注入普通分析、IntelAgent、事件驱动策略及 DecisionSignal 复盘摘要。
- [修复] A 股核心财报改用带交易所后缀的东方财富单股财务指标并增加同花顺摘要 fallback，修复报告期参数、财务字段映射、重复实时报价和慢补充接口覆盖已成功财报的问题。
- [改进] A 股个股资金流优先使用免费 efinance 历史资金流并正确计算 5/10 日累计；基本面各阶段分离来源、耗时与错误，低质量基本面不再保留虚假的策略归因权重。
- [新功能] A 股结构化事件增加未来 30 日限售解禁风险，RiskAgent 优先消费结构化事件后再做通用检索。
- [文档] 新增 A 股免费财报、资金流、事件能力与 Tushare 积分升级决策表，明确尚未接入的付费接口及成本决策证据。
- [改进] 持仓单股分析将 A 股、港股和美股的数量、成本、最新估值及浮盈亏以脱敏白名单注入策略 Prompt；补齐 `AAPL.US` 与 `AAPL` 等价匹配，并按市场约束持仓建议，账户标识与名称不进入 Prompt 或公开快照。
- [改进] Markdown 报告相关渲染路径压缩重复展示：默认报告、Jinja 同步模板与历史详情在已有行情快照时省略数据透视中的重复当前价，默认报告、Jinja 模板、单股通知与历史详情统一将归因权重改为单行，默认报告与 Jinja 模板将机器数据状态改为单行；保留风险正文、观察条件、点位与最强多空信号。
