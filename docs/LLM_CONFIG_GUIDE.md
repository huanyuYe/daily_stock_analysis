...
- 当前 `codex_cli` preset 在 `codex --ask-for-approval never exec` 后追加 `--ignore-user-config --disable shell_tool -c tools.web_search=false -c tools.view_image=false`，并继续使用 `--sandbox read-only --output-last-message <temp-file> -`。`--ignore-user-config` 只隔离用户级工具、MCP、hook 和行为配置，认证仍从既有 `CODEX_HOME` 获取；静态指令同时要求仅根据传入 prompt 生成最终文本，不调用工具或请求批准。这样普通分析不会因宿主 Codex 配置暴露了需批准的工具而中断，同时保留无人值守批准策略与只读边界。DSA 从临时文件读取最终响应；stdout 重复内容会从诊断预览和大小统计中剔除。官方依据见 [Codex config reference](https://developers.openai.com/codex/config-reference) 与 [Codex CLI command line options](https://developers.openai.com/codex/cli/reference)。当前 Lighthouse 部署的 `codex-cli 0.144.1` 已真实验证可以解析上述 preset 参数；本项目不据此声明更宽最低版本。如果 CLI 版本不支持 preset 参数，DSA 会返回结构化 `capability_unsupported` / `cli_contract_unsupported` 诊断，并在配置 backend fallback 时回退到 `litellm`。
