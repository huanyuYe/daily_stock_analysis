...
- The current `codex_cli` preset adds `--ignore-user-config --disable shell_tool -c tools.web_search=false -c tools.view_image=false` after `codex --ask-for-approval never exec`, then retains `--sandbox read-only --output-last-message <temp-file> -`. Ignoring user config isolates user-level tools, MCP servers, hooks, and behavior while authentication still comes from the existing `CODEX_HOME`; a static instruction also requires final-text-only generation from the supplied prompt without tool or approval requests. This prevents unattended analysis from inheriting a host tool that needs approval while preserving the non-interactive policy and read-only boundary. DSA reads the final response from the temporary file and removes duplicate stdout content from previews and size accounting. Official references: [Codex configuration reference](https://developers.openai.com/codex/config-reference) and [Codex CLI command line options](https://developers.openai.com/codex/cli/reference). The `codex-cli 0.144.1` installed on the current Lighthouse deployment has been live-verified to parse these preset arguments; DSA does not infer a wider minimum version range from that observation. Unsupported preset arguments return structured `capability_unsupported` / `cli_contract_unsupported` diagnostics and use the configured fallback when available.
