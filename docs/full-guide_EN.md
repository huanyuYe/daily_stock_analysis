...
Hong Kong and US stocks also have a structured regulatory-disclosure path. For US issuers, SEC-A reads EDGAR `submissions/CIK##########.json` filing metadata, while SEC-B reads `companyfacts/CIK##########.json` and selects the latest already-filed core XBRL fact by `filed_at`. For Hong Kong issuers, the public HKEXnews Title Search provides release time, title, issuer, and the original document link. Every normalized record retains `source_id`, `source_tier=official_regulator`, `verification_status=official_primary`, as-of time, and source diagnostics. The HKEXnews adapter is explicitly a public-page adapter, not a supported official API, and it does not download or parse PDF bodies. Configure this path with `REGULATORY_DISCLOSURES_ENABLED` (default `true`), `REGULATORY_FETCH_TIMEOUT_SEC` (default 8 seconds), and `SEC_EDGAR_USER_AGENT`. Individual source failures remain fail-open.

Watched US symbols now receive earnings-date and near-expiry option analysis by default. Using the project's existing `yfinance` dependency, the pipeline obtains the earnings date, available expiries, and the Call/Put chain for the most relevant expiry. An expiry within three days of earnings is highlighted as an event-risk window. The context and phase-specific pre-market, intraday, or post-market report section include the underlying move, Call/Put volume, Put/Call ratio, unusual volume/open-interest activity, premium, maximum loss per contract, breakeven, and an estimated probability of profit at expiry (PoP). This is delayed-capable, single-source aggregator data; the earnings date is not dual-source verified. PoP is a risk-neutral lognormal estimate using provider IV, not a historical win rate, and excludes fees, slippage, early exercise, volatility-surface changes, and earnings gaps. Configure it with `EARNINGS_OPTIONS_ENABLED`, `EARNINGS_OPTIONS_LOOKAHEAD_DAYS`, `EARNINGS_OPTIONS_EVENT_WINDOW_DAYS`, `EARNINGS_OPTIONS_PROFIT_PROBABILITY_THRESHOLD`, and `EARNINGS_OPTIONS_FETCH_TIMEOUT_SEC`. Disabling the feature or a fetch failure remains fail-open.

The local intelligence pool now also offers 15 curated RSS pilot templates covering macro, AI, software, semiconductors, EVs, solar, energy storage, healthcare, cybersecurity, and commercial space. Pilot templates are disabled by default and are never implicitly enabled by `NEWS_INTEL_AUTO_FETCH_ENABLED`. Set `NEWS_INTEL_AUTO_BOOTSTRAP_DEFAULTS=false` to make automatic refresh fetch only explicitly enabled sources instead of also creating the eight base templates. Once explicitly enabled, stock analysis supplements symbol and same-market items with deduplicated `market=global` evidence, still bounded by the configured time window and item limit. See [Intelligence Source MVP](intelligence-sources.md) for the source list, activation steps, and operating-cost boundary.
...
| `TICKFLOW_BATCH_SIZE` | Maximum symbols per TickFlow batch request for daily K-lines and realtime quotes; the current API limit is 5. | `5` | Optional |
...
> - The institution-analysis dimension now prefers exact-code A-share broker-research metadata from EastMoney for the latest 180 days. IntelAgent can call `get_research_reports` without a general search API key. Every rating and EPS estimate is labelled `source_tier=sell_side_aggregator` and `verification_status=single_source_opinion`; it is opinion evidence, never a company disclosure or verified realized result. Endpoint failures remain fail-open and fall back to the existing search providers.
...
> - The cross-market source comparison and SEC/HKEX expansion priorities are recorded in [Vibe-Research cross-market data-source assessment](cross-market-data-source-expansion.md) (Chinese).
> - **Field contracts**:
...
The final action is reconciled once more with holding state and the battle plan. `hold` is retained only for a confirmed position; an empty or unknown position becomes `watch`. Price levels attached to neutral actions are conditional entry references and risk controls for an existing holding, not immediate entry/add instructions. `reduce/sell` removes entry plans. QQ, Markdown, and DecisionSignal consumers use the final eight-state action as the authoritative classification.
...
#1390 P5 adds signal-level feedback, forward outcome evaluation, and stats sidecars. It does not extend the `decision_signals` main table and does not reuse `BacktestResult`, which is tied to `analysis_history_id`. `decision_signal_feedback` stores the latest `useful|not_useful` feedback per `signal_id` with optional reason/note/source. `decision_signal_outcomes` stores idempotent rows by `(signal_id, horizon, engine_version)`. New evaluations now use `engine_version=decision-signal-v2`; existing v1 rows remain intact and coexist with v2. Each outcome freezes `action/market/market_phase/source_type/source_agent/plan_quality/data_quality_level/holding_state` at evaluation time so historical stats are not rewritten by later live-join changes. Deleting history first finds `source_type=analysis` signals bound to the deleted history IDs, then removes their feedback/outcome sidecars.

v2 outcome evaluation supports daily-bar-verifiable `1d/3d/5d/10d`, where the window means the next 1/3/5/10 `StockDaily` bars after the anchor rather than natural-day expiration. It also supports `intraday` after signal expiry/session close by comparing evidence `current_price` with the final same-session daily bar. `anchor_date` first reads `metadata.market_phase_summary.session_date`, then falls back to `created_at.date()`; regular daily horizons require an exact anchor-date close with no previous-session fallback. Action mapping is `buy/add -> up`, `hold -> not_down`, and `reduce/sell/avoid -> not_up`. A `watch/alert` signal is directional only when evidence contains an explicitly bullish/strengthening or bearish/weakening trend, mapped to `up/not_up`; range-bound or absent trends stay non-directional. Missing prices, an open intraday session, a missing final bar, and insufficient forward bars are recoverable unable states. `swing/long`, non-directional signals, and missing anchor dates remain terminal unable states. Automatic extraction may receive runtime `portfolio_context.quantity`; it writes only low-sensitive `holding_state=holding|empty|unknown` into metadata for outcome snapshots, never quantity, account, or cost.

P5 extends the existing Web `/decision-signals` page instead of adding a new navigation page or BacktestPage entry. The filter area shows current outcome-engine stat cards; the details drawer lazily loads outcomes and accepts useful/not useful feedback. `POST /api/v1/decision-signals/outcomes/run` remains available for explicit runs. When `BACKTEST_ENABLED=true`, every successful analysis now first advances up to 200 missing or retryable v2 outcomes and then runs the existing history backtest; failures are isolated between both feedback loops and from report generation. Batch runs prioritize missing outcomes first and then retry recoverable unable rows, so completed or terminal-unable newest signals do not keep consuming the `limit`.
...
| Variable | Default | Description |
|----------|---------|-------------|
| `BACKTEST_ENABLED` | `true` | Whether to advance DecisionSignal outcomes and run history backtests after daily analysis |
