...
The local intelligence pool now also offers 15 curated RSS pilot templates covering macro, AI, software, semiconductors, EVs, solar, energy storage, healthcare, cybersecurity, and commercial space. Pilot templates are disabled by default and are never implicitly enabled by `NEWS_INTEL_AUTO_FETCH_ENABLED`. Once explicitly enabled, stock analysis supplements symbol and same-market items with deduplicated `market=global` evidence, still bounded by the configured time window and item limit. See [Intelligence Source MVP](intelligence-sources.md) for the source list, activation steps, and operating-cost boundary.
