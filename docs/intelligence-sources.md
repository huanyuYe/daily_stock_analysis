...
> 与 A 股结构化标的事件的关系：本页描述的是可配置、可持久化的 RSS/Atom/NewsNow 情报池。个股分析另有一条按需、只读、fail-open 的结构化事件链路，通过 AkShare 拉取巨潮资讯公告和东方财富个股新闻；它不创建 `intelligence_sources`，也不把原始事件写入 `intelligence_items`。两条链路最终都会补充分析上下文，但来源契约与持久化边界不同。
