...
- 小红书 URL、昵称、ID 与二维码路径可由 `SHARE_IMAGE_XIAOHONGSHU_*` 配置覆盖；全部留空时显示内置二维码和昵称 `@霸天土小豆`，默认不展示数字 ID。二维码转图时嵌入 HTML，不依赖外部图片服务或运行时网络。
- 模板只展示报告已有的 0–100 评分、八态动作和 `battle_plan.sniper_points` 点位，不生成额外评分或多空占比。`buy/add` 展示入场、止损和目标；`hold/watch/avoid/alert` 将买入点明确标为条件入场参考，将止损和目标标为已有持仓的风控/止盈观察位；`reduce/sell` 不展示开仓或加仓计划。
