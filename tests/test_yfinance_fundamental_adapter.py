...
def _recent_quarterly_dividend_dates(tz: str) -> pd.DatetimeIndex:
    """Return four deterministic-in-window dates for rolling TTM assertions."""
    today = pd.Timestamp.now(tz=tz).normalize()
    return pd.DatetimeIndex(
        [today - pd.Timedelta(days=days) for days in (270, 180, 90, 1)]
    )
...
class TestYfinanceFundamentalAdapter(unittest.TestCase):
    def test_populates_growth_earnings_dividend_boards_for_us_stock(self) -> None:
        ...
        dividend_dates = _recent_quarterly_dividend_dates("America/New_York")
        dividends = pd.Series(
            [0.26, 0.26, 0.26, 0.27],
